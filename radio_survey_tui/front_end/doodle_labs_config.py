...
class DoodleScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Center(id="overlay-container"):
            with Middle():
                ...
                main_container.border_subtitle = "doodleOS"

                with main_container:
                    yield Static("◆ doodle config menu ◆", id="header-text")
                    yield Static("─" * 54)
                    yield Label("doin the doodle...", id="status-box-doodle")
                    yield ListView(
                        ListItem(Label("> view doodle JSON configuration"), id="viewdoodleconf"),
                        ListItem(Label("> doodle radio configuration"), id="doodleconf"),
    ...
    def on_list_view_selected(self, event: ListView.Selected) -> None:
        ...
        if choice == "back":
            self.app.pop_screen()
        elif choice == "viewdoodleconf":
            with self.app.suspend():
                subprocess.call(['vim', '/agility/doodle_labs_radio_configuration_and_setup/doodle_labs_radio_configuration_profile.json'])
        elif choice == "doodleconf":
            ...
            self.run_survey_script("/agility/doodle_labs_configuration_and_setup/configure_radio.sh") # Replace with your actual shell script!
        else:
